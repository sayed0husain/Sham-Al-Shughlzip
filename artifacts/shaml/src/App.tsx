...
function BottomSection() {
  ...
  const totalProjects = games.length + sites.length;
      ...
      <div className="flex flex-col md:flex-row gap-12 md:gap-16 items-start">
        ...
        <div className="flex-1">
          <div className="grid grid-cols-2 gap-4">
            ...
            <StatCard label="الأيادي العاملة في شمل" value={members.length || 0} delay="delay-300" />
            <StatCard label="ألعاب مبتكرة" value={games.length} delay="delay-400" />
            <StatCard label="مواقع منشورة" value={sites.length} delay="delay-500" />
          </div>
        </div>
      </div>
    </section>
...
function TeamSection() {
  ...
  return (
    <section className="w-full" style={{ borderTop: "1px solid hsl(30,12%,88%)" }}>
      <div className="max-w-6xl mx-auto px-6 py-16">
        <h2 className="text-2xl md:text-3xl font-bold mb-8 text-right" style={{ fontFamily: FONT_H, color: "hsl(20,10%,15%)" }}>
          الأيادي العاملة
        </h2>

        {/* VIP members pinned at top with extra margin */}
        {vip.length > 0 && (
        ...
        {/* Separator only if both groups exist */}
        {vip.length > 0 && rest.length > 0 && (
          <div className="mb-6" style={{ height: 1, backgroundColor: "hsl(30,12%,88%)" }} />
        )}
  ...
  return (
    <section className="w-full" style={{ borderTop: "1px solid hsl(30,12%,88%)", backgroundColor: "hsl(30,15%,96%)" }}>
      <div className="max-w-4xl mx-auto px-6 py-16 flex flex-col gap-10">
        ...
        {statements.map((stmt) => (
          <article key={stmt.id} className="flex flex-col gap-4 pb-10"
            style={{ borderBottom: "1px solid hsl(30,12%,88%)" }}>
            <h3 style={{ fontFamily: FONT_H, fontSize: "1.4rem", color: "hsl(20,10%,12%)", textAlign: "right" }}>
              {stmt.title}
            </h3>
        ...
        <div className="flex items-center gap-3">
          {contact.whatsapp && (
            <a href={`https://wa.me/${formatWhatsapp(contact.whatsapp)}`} target="_blank" rel="noopener noreferrer"
              ...
              style={{ backgroundColor: "#25D366" }} aria-label="واتساب">
          ...
          {contact.email && (
            <a href={`mailto:${contact.email}`}
              className="w-9 h-9 rounded-xl flex items-center justify-center transition-transform hover:scale-110"
              style={{ backgroundColor: ACCENT }} aria-label="البريد الإلكتروني">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="white">
